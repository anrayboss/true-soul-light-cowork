...
def run_ocr():
    ...
    images = [f for f in os.listdir(folder) if f.lower().endswith(supported_extensions)]
    images.sort(key=natural_sort_key)
    
    print(f"Loading EasyOCR reader...")
    reader = easyocr.Reader(['ch_tra', 'en'])
    
    output_txt_path = os.path.join(folder, "ocr_raw_results.txt")
    ...
    with open(output_txt_path, "w", encoding="utf-8") as out:
        for idx, img_name in enumerate(images):
            ...
            # then x coordinate
            results_sorted = sorted(results, key=lambda x: (x[0][0][1] // 15, x[0][0][0]))
